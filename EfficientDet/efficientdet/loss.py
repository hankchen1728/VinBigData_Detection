...
class FocalLoss(nn.Module):
    ...
    def forward(self, classifications, regressions, anchors, annotations, **kwargs):
        ...
        if imgs is not None:
            ...
            imgs = imgs.permute(0, 2, 3, 1).cpu().numpy()  # to channel last
            if imgs.shape[-1] != 3:  # not RGB image
                # default choosing first channel to plot
                imgs = np.repeat(imgs[..., 0], repeats=3, axis=-1)
